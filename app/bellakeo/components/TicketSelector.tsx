  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-6xl mx-auto px-4 py-8">
      {tickets.map((ticket, index) => {
        ...
        const colorClasses = ticket.color; // e.g., border-blue-500, glow-blue, etc.
        ...
        return (
          <motion.div
            ...
            className={`relative flex flex-col justify-between p-6 rounded-2xl border bg-black/40 backdrop-blur-xl cursor-pointer transition-all duration-300 ${
              isSelected
                ? 'border-indigo-500 shadow-[0_0_30px_rgba(99,102,241,0.3)] bg-gradient-to-b from-indigo-950/20 to-black/60'
                : 'border-white/10 hover:border-indigo-500/40 hover:shadow-[0_0_20px_rgba(99,102,241,0.1)]'
            } overflow-hidden`}
          ...
          >
            ...
            <div>
              <div className="flex items-center gap-3 mb-4">
                <div className={`p-2.5 rounded-xl bg-white/5 border border-white/10 ${
                  isSelected ? 'text-indigo-400 border-indigo-500/30' : 'text-zinc-400'
                }`}>
                  {ticket.icon}
                </div>
            ...
            >
              {isSelected ? 'Seleccionado' : ticket.id === 'table' ? 'Reservar mesa' : 'Seleccionar'}
            </motion.button>
          </motion.div>
        );
      })}
